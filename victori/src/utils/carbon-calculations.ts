...
export const TREE_SPECIES_DB: Record<string, TreeSpecies> = {
  teak: {
    ...
    barkPattern: 'Rough, fibrous, grey-brown with shallow longitudinal fissures',
  },
  neem: {
    name: 'Neem',
    ...
    barkPattern: 'Dark grey, thick, deeply fissured with criss-cross pattern',
  },
  banyan: {
    name: 'Banyan',
    ...
    barkPattern: 'Smooth light grey bark with prominent aerial roots',
  },
  mango: {
    name: 'Mango',
    ...
    barkPattern: 'Dark grey-brown, rough with deep vertical fissures and thick plates',
  },
  peepal: {
    name: 'Peepal',
    ...
    barkPattern: 'Smooth whitish-grey bark becoming scaly with age',
  },
  eucalyptus: {
    name: 'Eucalyptus',
    ...
    barkPattern: 'Smooth cream-white bark that peels in long strips',
  },
  coconut: {
    name: 'Coconut',
    ...
    avgHeight: 20,
    growthRate: 'Medium',
    co2AbsorptionRate: 30.0,
    barkPattern: 'Smooth grey ringed trunk with regular leaf scar bands',
  },
  oak: {
    name: 'Oak',
    ...
    barkPattern: 'Deeply furrowed dark grey bark with thick ridges',
  },
  pine: {
    name: 'Pine',
    ...
    barkPattern: 'Reddish-brown scaly bark with deep fissures at base',
  },
}
 ...
 * D   = DBH (cm) - Diameter at Breast Height
 * H   = total height (m)
 *
...
export function calculateChaveBiomass(
  ...
  return 0.0673 * Math.pow(woodDensity * dbh * dbh * height, 0.976)
}

/**
 * Full Chave analysis: biomass -> carbon -> CO2 -> credits -> USD
 */
export function calculateChaveResult(species: TreeSpecies): ChaveResult {
  const agb = calculateChaveBiomass(species.woodDensity, species.avgDbh, species.avgHeight)
  const carbonStored = agb * species.carbonFraction
  const co2Sequestered = carbonStored * (44 / 12) // molecular weight ratio CO2/C
  const carbonCredits = co2Sequestered / 1000     // 1 credit = 1 metric ton CO2
  const creditValueUSD = carbonCredits * 40       // ~$40/credit average market price
...
/**
 * Adjusts carbon sequestration based on real-time environmental sensor data.
 * Better conditions (optimal temp, humidity, sunlight, soil, pH) yield higher multiplier.
 */
function getEnvironmentalMultiplier(sensor: SensorData): number {
  let multiplier = 1.0

  // Temperature (optimal 20-30 C for photosynthesis)
  ...
  // Sunlight / PAR (optimal 400-800 micromol/m^2/s)
...
// ==================== CARBON CREDIT SCORE CALCULATOR ====================

/**
 * The master Carbon Credit Score calculation.
 * Combines Chave biomass equation with real-time sensor environmental conditions.
 *
 * Score breakdown (weighted):
 *   35% - Sequestration Score (Chave biomass * environmental multiplier)
 *   25% - Emission Offset (CO2 absorbed vs CO2 emitted by vehicles)
 *   25% - Environmental Health (sensor conditions quality)
...
export function calculateCarbonCreditScore(
  ...
  const baseSequestration = Math.min((chaveResult.co2Sequestered / 500) * 100, 100)
  const sequestrationScore = Math.round(baseSequestration * envMultiplier)

  // Emission offset score
  const emissionOffset = Math.round(
    Math.min((sensor.co2Absorbed / Math.max(sensor.co2Emitted, 1)) * 100, 100)
  )

  // Environmental health
  const tempScore = sensor.temperature >= 18 && sensor.temperature <= 32 ? 100 : 50
  const humidScore = sensor.humidity >= 30 && sensor.humidity <= 80 ? 100 : 50
  const lightScore = sensor.lightIntensity >= 200 && sensor.lightIntensity <= 1000 ? 100 : 50
  const soilScore = sensor.soilMoisture >= 20 && sensor.soilMoisture <= 80 ? 100 : 50
  const environmentalHealth = Math.round((tempScore + humidScore + lightScore + soilScore) / 4)

  // Oxygen production score
  const oxygenProduction = Math.round(Math.min((sensor.o2Released / 200) * 100, 100))
 ...
 * In production, real data comes from ESP32 via WebSocket or HTTP.
 */
export function generateSimulatedSensorData(): SensorData {
  return {
    temperature: 26 + (Math.random() - 0.5) * 6,
    humidity: 62 + (Math.random() - 0.5) * 20,
